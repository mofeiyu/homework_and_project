...
class FbTreeNode:
    def __init__(self, attribute_no, parent, children, attribute_times=0):
...
class FbTree:
    def __init__(self):
        ...
        self.head_null_node = FbTreeNode()
    ...
    def create_fb_tree(self):
        for (user_id, page_ls) in self.new_data:
            ...
            for each_page in page_ls:    # 遍历每一项用户点击页面的列表
                if not self.head_null_node.children:    # 如果是第一次，头结点的子节点尚为空值，则从头开始搞
                    self.head_null_node.children.append(FbTreeNode(each_page, self.head_null_node.children, None, attribute_times=1))
                    current_node = self.head_null_node.children[0]
                    self.item_head_table[self.sort_list.index()]
                    ...
                    if each_page == current_node.attribute_no:    # 当前节点之前已经有过了
                        ...
                        if current_node.children:    # 如果子节点非空，则寻找是否应该使用下面那个子节点
                            ...
                            current_node.children.append(FbTreeNode(each_page, current_node, None, attribute_times=1))
